...
class RiotWatcher:
    ...
    def base_grequest(self, url, region, static=False, **kwargs):
        if region is None:
            region = self.default_region
        args = {'api_key': self.key}
        for k in kwargs:
    ...
    def send_async_requests(self, grequest_list):
        responses = [response for response in grequests.map(grequest_list)]
        map(raise_status, responses)
        for lim in self.limits:
                lim.add_request()
        return [json.loads(response._content) for response in responses]
    ...
    def _match_grequest(self, end_url, region, **kwargs):
        return self.base_grequest(
            'v{version}/match/{end_url}'.format(
                version=api_versions['match'],
                end_url=end_url
            ),
    ...
    def get_match_async(self, match_id, region=None, include_timeline=False):
        return self._match_grequest(
            '{match_id}'.format(match_id=match_id),
            region,
            includeTimeline=include_timeline
        )
