...
from config import ES_NODES, BLANK_FILTERS, CLUSTER_DATAS
...
def gen_feature_bar(feature, user, cluster):
    res = {
            ...
            "cluster": CLUSTER_DATAS[cluster][feature],
            "genpop": CLUSTER_DATAS["all"][feature]
        }
    return res
...
"""
    ...
    #filters: { "and": [], "or": [], "not": [] }
"""
def gen_histo(feature, user, cluster, filters):
    b = Brief(filters, { "key":feature["name"], "choices": feature["choices"], "format":"PCTG" }, BLANK_FILTERS)
    return b.to_json()

f = { "name": "pMinions", "choices": [[float(i)/10, float(i+1)/10] for i in range(10)] }
print gen_histo(f, {}, "", BLANK_FILTERS)